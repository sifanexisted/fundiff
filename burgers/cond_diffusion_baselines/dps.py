...
class AttentionOp(nn.Module):
    ...
    def __call__(self, q: jnp.ndarray, k: jnp.ndarray) -> jnp.ndarray:
        ...
        w = jax.nn.softmax(attn, axis=2).astype(dtype)
        return w
...
class UNetBlock(nn.Module):
    ...
    @nn.compact
    def __call__(self, x, emb, *, context=None, deterministic: bool = False):
        heads = 0
        if self.attention:
            heads = (self.num_heads
                     if self.num_heads is not None
                     else self.out_channels // self.channels_per_head)
        ...
        if heads > 0:
            ...
            h = GroupNorm(num_channels=C, eps=self.eps)(x)
            qkv = Conv2d(
                in_channels=C,
                out_channels=C * 3,
                kernel=1,
                **(self.param_init_attn or self.param_init),
            )(h) # (B, 3C, H, W)
            qkv = qkv.reshape(B * heads, cp_head * 3, -1) # (B*H,3*Cp,HW)
            q, k, v = jnp.split(qkv, 3, axis=1) # (B*H,Cp,HW) each

            w = AttentionOp()(q, k) # (B*H, HW, HW)

            a = jnp.einsum("nqk,nck->ncq", w, v) # (B*H, Cp, HW)
            a = a.reshape(B, C, H, W)  # back

            # cross attention with context serving as key and values
            if context is not None:
                # context shape: (B, N, D)
                B_ctx, N, D = context.shape

                # Project context to get keys and values
                # Linear projection: D -> 2*C (for key and value)
                kv_ctx = nn.Dense(
                    features=C * 2,
                    **(self.param_init_attn or self.param_init),
                )(context)  # (B, N, 2*C)
                ...
                kv_ctx = kv_ctx.reshape(B, N, 2, heads, cp_head)  # (B, N, 2, heads, Cp)
                kv_ctx = kv_ctx.transpose(2, 0, 3, 4, 1)  # (2, B, heads, Cp, N)
                kv_ctx = kv_ctx.reshape(2, B * heads, cp_head, N)  # (2, B*H, Cp, N)
                k_ctx, v_ctx = kv_ctx[0], kv_ctx[1]  # (B*H, Cp, N) each

                # Cross attention: query from x, key and value from context
                w_cross = AttentionOp()(q, k_ctx)  # (B*H, HW, N)

                a_cross = jnp.einsum("nqk,nck->ncq", w_cross, v_ctx)  # (B*H, Cp, HW)
                a_cross = a_cross.reshape(B, C, H, W)  # back
        ...
        for name, blk in self.enc.items():
            if "aux_down" in name:
                ...
                if isinstance(blk, UNetBlock):
                    x = blk(x, emb, context=context, deterministic=deterministic)
                else:
                    x = blk(x)
                skips.append(x)
